...
function calculateResults(bill: Bill, people: string[], assignments: Assignment): Record<string, PersonResult> {
  ...
  bill.items.forEach((item) => { const assigned = assignments[item.id] ?? []; const names = assigned.includes("everyone") ? people : assigned; const share = names.length ? Math.floor(item.totalPrice / names.length) : 0; names.forEach((name, index) => { const amount = index === names.length - 1 ? item.totalPrice - share * (names.length - 1) : share; results[name].subtotal += amount; results[name].items.push({ name: item.name, share: amount }); }); });
  const consumption = people.reduce((sum, person) => sum + results[person].subtotal, 0);
  people.forEach((person) => { const ratio = consumption ? results[person].subtotal / consumption : 0; results[person].discount = Math.round(bill.discount * ratio); results[person].serviceCharge = Math.round(bill.serviceCharge * ratio); results[person].tax = Math.round(bill.tax * ratio); results[person].total = results[person].subtotal - results[person].discount + results[person].serviceCharge + results[person].tax; });
  return results;
}
...
export function ResultsScreen({ bill, people, assignments, onBack, onStartOver }: { bill: Bill; people: string[]; assignments: Assignment; onBack: () => void; onStartOver: () => void }) {
  ...
  const total = bill.subtotal - bill.discount + bill.serviceCharge + bill.tax;
  return (
